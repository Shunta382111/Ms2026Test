...
namespace Framework.Core.Event
{
    ...
    /// </summary>
    public abstract class IEvent<TEvent> : IEnter, IUpdate, IExit
        where TEvent : IEvent<TEvent>
    {
        private EventContext<TEvent> _ctx;

        internal void __Bind(EventContext<TEvent> ctx) => _ctx = ctx;
        ...
        /// 子イベントを追加
        /// </summary>
        public void AddChild<TEnum>(TEnum id, TEvent ev, int priority = 0) where TEnum : Enum
